function result = optimize_OS1_BO_GitHub(cfg)
if nargin < 1
    cfg = struct();
end

% Axes handle for GUI embedding (optional)
...
% OS1 multi-LiDAR optimization with trajectory, fixed-roll option, and hard symmetry mirroring.
% - Symmetry: optimize "masters" on one side; auto-mirror across YZ plane (x -> -x, pitch -> -pitch)
% - Roll: optionally fixed (e.g., 0°), or optimized per master
% - Uses voxelized coverage, overlap window reward, and angular diversity reward
%
% Output:
...
%% -------------------- OS1 scan model --------------------
% Default OS1 settings (override via cfg if desired)
OS1_VERT_FOV_DEG    = 42.4;   % approx vertical FoV used in your draft
OS1_VERT_BEAMS      = 64;     % channels
OS1_AZ_SAMPLES      = 180;    % rays per pose (speed knob)
OS1_SCANS           = 1;      % slices per pose
BEAM_DIVERGENCE_DEG = 0.18;   % jitter for final rebuild/reporting
MAX_RANGE           = 120;    % m

% Allow overrides (recommended if you have exact OS1 channel angles)
if isfield(cfg,'OS1_VERT_FOV_DEG');  OS1_VERT_FOV_DEG = cfg.OS1_VERT_FOV_DEG; end
if isfield(cfg,'OS1_VERT_BEAMS');    OS1_VERT_BEAMS   = cfg.OS1_VERT_BEAMS; end
if isfield(cfg,'OS1_AZ_SAMPLES');    OS1_AZ_SAMPLES   = cfg.OS1_AZ_SAMPLES; end
if isfield(cfg,'OS1_SCANS');         OS1_SCANS        = cfg.OS1_SCANS; end
if isfield(cfg,'MAX_RANGE');         MAX_RANGE        = cfg.MAX_RANGE; end
if isfield(cfg,'BEAM_DIVERGENCE_DEG'); BEAM_DIVERGENCE_DEG = cfg.BEAM_DIVERGENCE_DEG; end

% Vertical angles
% If you have true OS1 channel angles, pass them as cfg.OS1_VERT_ANGLES_DEG (1x64)
if isfield(cfg,'OS1_VERT_ANGLES_DEG')
    vert_angles = cfg.OS1_VERT_ANGLES_DEG(:).';  % row vector
else
...
%% -------------------- Objective & BO settings --------------------
VOX                  = 0.5;   % m
OVERLAP_MIN          = 0.05;
OVERLAP_MAX          = 0.10;
MIN_ANGULAR_DIV      = 30;    % deg (between local +Z axes of mounts)
LAMBDA               = 2000;  % per-sensor penalty (tune)
BEAM_DIV_BO          = 0;     % determinism during BO
MASTERS_MAX          = 5;     % masters; mirrored copies are added if symmetry enabled
MAX_EVALS            = 300;
INIT_SAMPLES         = 12;

% Override defaults with GUI values when provided
...
if isfield(cfg,'MAX_EVALS');       MAX_EVALS = cfg.MAX_EVALS; end
if isfield(cfg,'INIT_SAMPLES');    INIT_SAMPLES = cfg.INIT_SAMPLES; end

% Apply GUI symmetry / roll settings correctly
...
if isfield(cfg,'FIX_ROLL_DEG');      FIX_ROLL.DEG    = cfg.FIX_ROLL_DEG; end

meta     = make_vox_meta(box, VOX);
T_B_list = build_straight_path(box, R_backpack, Z_BACKPACK, MOVE_AXIS, PATH_LEN, PATH_STEP, MARGIN);

%% -------------------- Optimizable variables --------------------
% Optimize N_masters (integer) and for each master: x,y,z,pitch,roll
ov = optimizableVariable('N_masters',[1 MASTERS_MAX],'Type','integer');
mk = @(n,r) optimizableVariable(n, r);
for s = 1:MASTERS_MAX
    ov(end+1) = mk(sprintf('xM_%d',s), b.x);
    ov(end+1) = mk(sprintf('yM_%d',s), b.y);
    ov(end+1) = mk(sprintf('zM_%d',s), b.z);
    ov(end+1) = mk(sprintf('pM_%d',s), b.pitch);
    ov(end+1) = mk(sprintf('rM_%d',s), b.roll);
end

%% -------------------- BO objective (minimization) --------------------
obj = @(tbl) evaluate_layout_BO( ...
...
if isfield(cfg,'FromGUI') && cfg.FromGUI
    boPlotFcn = {@(results,state) bo_convergence_gui(results,state,cfg.ConvAxes)};
else
    boPlotFcn = {@plotObjectiveModel, @plotMinObjective};
end

% Early-stop settings (same pattern as your final VLP code)
stallWindow = 50;     % evals to look back
tolAbs      = 5;      % absolute improvement threshold in f = -J
tolRel      = 1e-5;   % relative improvement threshold
useRelTol   = true;
minEvals    = 40;     % prevent premature stopping

results = bayesopt(obj, ov, ...
    'IsObjectiveDeterministic', true, ...
    'AcquisitionFunctionName', 'expected-improvement-plus', ...
    'MaxObjectiveEvaluations', MAX_EVALS, ...
    'NumSeedPoints', INIT_SAMPLES, ...
    'ExplorationRatio', 0.4, ...
    'UseParallel', false, ...
    'PlotFcn', boPlotFcn, ...
    'OutputFcn', @(res,state) boEarlyStop(res,state,stallWindow,tolAbs,tolRel,useRelTol,minEvals), ...
    'Verbose', 1);
...
for s = 1:N
    th      = theta_list{s};
    M_local = theta_to_mount_local(th);

    cov_s = simulate_os1_bitmap( ...
...
fid  = fopen('C:..........\BO_best_mounts_os1_symm.json','w'); fwrite(fid,json); fclose(fid);
fprintf('✓ Exported JSON: BO_best_mounts_os1_symm.json\n');

toc

%% -------------------- Visualization --------------------
if embedPlots && ~isempty(ax)
    plot_multi_setup_with_path_OS1(box, mounts, perSensorSets, meta, T_B_list, OS1_VERT_FOV_DEG, ax);
else
    plot_multi_setup_with_path_OS1(box, mounts, perSensorSets, meta, T_B_list, OS1_VERT_FOV_DEG);
end
drawnow; shg;

%% -------------------- Per-sensor CSV --------------------
[uniq_counts, overlap_count, ~, contrib_counts] = coverage_stats(perSensorSets);  
...
fmtHeader = '\n=== PER-SENSOR MOUNT SUMMARY (OS1-64, symmetric) ===\n%4s  %8s %8s %8s   %8s %8s %8s   %8s %10s %9s\n';
...
writetable(T, 'mount_summary_os1_BO.csv');
fprintf('✓ Saved CSV: mount_summary_os1_BO.csv\n');
...
for s = 1:Nm
    ...
    % position: x -> -x
    % orientation: pitch -> -pitch, roll unchanged (unless fixed)
    thS = thM;
    thS(1) = -thM(1);
    thS(4) = -thM(4);
...
function M = theta_to_mount_local(theta)
...
function cov = simulate_os1_bitmap(M_local, T_B_list, box, maxR, vert_angles_deg, az_samp, scans, div_deg, meta)
cov = false(meta.n,1);
...
for a = 1:az_samp
    for v = 1:nv
        idx = idx+1;
        D_local(:,idx) = angles_to_dir(az_list(a), vert_angles_deg(v));
    end
end
...
function d = angles_to_dir(az_deg, el_deg)
...
function plot_multi_setup_with_path_OS1(box, mounts_local, perSensorSets, vox_meta, T_B_list, vFOVdeg, ax)
...
title(ax,'Multi-OS1-64 Coverage with Trajectory (Symmetry/Fix-Roll)');
ax.Color = 'none';
view(ax,45,25);
...
for s=1:S
    ...
    for i=1:numel(ks)
        k = ks{i};
        if cnt(k)==1, uniq{end+1} = k; end %#ok<AGROW>
    end
...
if ~isa(ax,'matlab.ui.control.UIAxes')
    ...
    function draw_sensor_glyph(T, color, ~)
...
function stop = bo_convergence_gui(results,state,axConv)
stop = false;
if nargin < 3 || isempty(axConv) || isempty(results) || isempty(results.ObjectiveMinimumTrace)
    return;
end
x = results.ObjectiveMinimumTrace;
evals = 1:numel(x);
...
plot(axConv, evals, x, 'b-o', 'LineWidth',1.4 );
if ~isempty(results.EstimatedObjectiveMinimumTrace)
    est = results.EstimatedObjectiveMinimumTrace;
    plot(axConv, evals, est, 'g-*', 'LineWidth',1.4 );
end
xlabel(axConv, 'Function evaluations');
ylabel(axConv, 'Min objective');
title(axConv, 'BO Convergence');
legend(axConv,'Location','best');
drawnow;
end

function stop = boEarlyStop(res, state, stallWindow, tolAbs, tolRel, useRelTol, minEvals)
stop = false;
persistent bestTrace
...
switch state
    ...
    case 'iteration'
        if isprop(res, "ObjectiveMinimumTrace") && ~isempty(res.ObjectiveMinimumTrace)
            bestNow = res.ObjectiveMinimumTrace(end);
        else
            bestNow = min(res.ObjectiveTrace);
        end

        bestTrace(end+1) = bestNow;  
        k = numel(bestTrace);

        if k < max(minEvals, stallWindow+1)
            return;
        end

        bestPrev    = bestTrace(k - stallWindow);
        improvement = bestPrev - bestNow;  % minimization: positive means improvement

        if useRelTol
            relImp = improvement / max(1, abs(bestPrev));
            stop = (relImp < tolRel);
            if stop
                fprintf('[BO Early Stop] relImp=%.3g over last %d evals. Stopping.\n', relImp, stallWindow);
            end
        else
            stop = (improvement < tolAbs);
            if stop
                fprintf('[BO Early Stop] absImp=%.6g over last %d evals. Stopping.\n', improvement, stallWindow);
            end
        end
